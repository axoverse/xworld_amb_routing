...
class UnidotMeshRenderer:
	...
	func create_godot_node_orig(state: RefCounted, new_parent: Node3D, component_name: String) -> Node:
		...
		var mesh_meta: Resource = meta.lookup_meta(mesh_ref)
		if mesh_meta == null and not is_stripped and not gameObject.is_stripped:
			log_debug("Skipping " + component_name + " node: no mesh resource found")
			return null
...
class UnidotMonoBehaviour:
	...
	# ---- AxNode3D MonoBehaviour GUID constants ----
	const GUID_THINGLINK := "d6cc4a6afe94ab74dbe81b3d05a697ce"
	const GUID_LAYER := "03018235bf6b89247a5855273f08d67f"
	const GUID_LOCKTRANSFORM := "4bc57520dae7d4c44b171809c56e7da7"
	const GUID_PIVOTOVERRIDE := "1bab28f11fb77ce46abb526d9eef2544"
	const GUID_ROUTINGWAYPOINT := "6fd437f5c0da6c0438d2e7610f29d28d"
	const GUID_THING3DLABEL := "cc8d9b8704344881b07a1ca717f26e96"
	const GUID_THINGEDGE := "4aaadace25d630d428b950b240af170f"
	const GUID_TIMESTATEMANAGER := "13b329facb4e93e40b5406fa9f979a19"
	const GUID_XTIMESTATE := "801e0c348a685404295bb872b661811a"

	const AXNODE_GUIDS: Array[String] = [
		GUID_THINGLINK, GUID_LAYER, GUID_LOCKTRANSFORM, GUID_PIVOTOVERRIDE,
		GUID_ROUTINGWAYPOINT, GUID_THING3DLABEL, GUID_THINGEDGE,
		GUID_TIMESTATEMANAGER, GUID_XTIMESTATE,
	]

	# ---- Enum lookup tables (order must match Unity C# enum definitions) ----
	const PHYSICAL_TYPES: Array[String] = [
		...
		"RoomGroup","HvacArea","SanitationArea",
	]

	const THING_EDGE_TYPES: Array[String] = [
		"none","any","type","subtype","child","sibling","member","owned","dependent",
		"instance","subject","location","reference","element","valuetype","external",
	]

	const LOCKTRANSFORM_XZ: Array[String] = ["None", "Zero"]
	const LOCKTRANSFORM_Y: Array[String] = ["None", "Zero", "FalseCeiling", "Ceiling"]

	const TIMESTATE_ENABLE_TYPES: Array[String] = ["NoChange", "Enable", "Disable", "Flip"]
	const TIMESTATE_TRANSFORM_TYPES: Array[String] = ["Position", "Rotation", "Scale"]

	func get_godot_type() -> String:
		return "GDScript"

	func _relative_node_path(from_path: NodePath, to_path: NodePath) -> NodePath:
		var from_parts: PackedStringArray = str(from_path).split("/")
		var to_parts: PackedStringArray = str(to_path).split("/")
		var common: int = 0
		while common < from_parts.size() and common < to_parts.size() and from_parts[common] == to_parts[common]:
			common += 1
		var up_count: int = from_parts.size() - common
		var rel_parts: PackedStringArray = []
		for i in range(up_count):
			rel_parts.append("..")
		for i in range(common, to_parts.size()):
			rel_parts.append(to_parts[i])
		return NodePath("/".join(rel_parts))

	func _get_axnode_guid() -> String:
		var guid: Variant = monoscript[2]
		if typeof(guid) == TYPE_STRING and guid in AXNODE_GUIDS:
			return guid
		return ""

	## Infer the AxNode3D GUID from property names in uprops.
	## Used as a fallback during prefab modification processing, where the virtual
	## MonoBehaviour object does not carry the original m_Script reference.
	func _infer_axnode_guid_from_uprops(uprops: Dictionary) -> String:
		# ThingLink has the most distinctive properties
		for key in ["ThingLabelOverride", "ThingNameOverride", "PhysicalType", "ThingInstanceLabel", "ObjectInstanceGuid", "PrefabPath"]:
			if uprops.has(key):
				return GUID_THINGLINK
		# LockTransform - check before Layer since both could have "Layers" in theory
		if uprops.has("LockX") or uprops.has("LockY") or uprops.has("LockZ") or uprops.has("Messages"):
			return GUID_LOCKTRANSFORM
		# Layer
		if uprops.has("Layers"):
			return GUID_LAYER
		# PivotOverride (Rotation + CamDistanceMultiplier without ThingLink context)
		if uprops.has("CamDistanceMultiplier"):
			return GUID_PIVOTOVERRIDE
		if uprops.has("Rotation") and not uprops.has("ThingLabelOverride"):
			return GUID_PIVOTOVERRIDE
		# ThingEdge
		if uprops.has("Vertex1") or uprops.has("Vertex2"):
			return GUID_THINGEDGE
		if uprops.has("Type") and uprops.size() <= 3:
			return GUID_THINGEDGE
		# TimeStateManager
		if uprops.has("SceneDateTime") or uprops.has("TestStateChanges"):
			return GUID_TIMESTATEMANAGER
		# XTimeState
		if uprops.has("TimeStateChanges"):
			return GUID_XTIMESTATE
		return ""

	func create_godot_node(state: RefCounted, new_parent: Node3D) -> Node:
		var guid: String = _get_axnode_guid()

		if not guid.is_empty():
			# Attach AxNode3D script to parent node if not already present
			if new_parent.get_script() != AxNode3D:
				new_parent.set_script(AxNode3D)
			# Enable the corresponding feature toggle
			match guid:
				GUID_THINGLINK:
					new_parent.useThingLink = true
				GUID_LAYER:
					new_parent.useLayers = true
				GUID_LOCKTRANSFORM:
					new_parent.useTransformLock = true
				GUID_PIVOTOVERRIDE:
					new_parent.usePivotOverride = true
				GUID_ROUTINGWAYPOINT:
					new_parent.isRoutingWaypoint = true
				GUID_THING3DLABEL:
					new_parent.useLabel3D = true
				GUID_THINGEDGE:
					if not new_parent.useEdges:
						new_parent.EdgeFlags = 0
					new_parent.useEdges = true
					new_parent.useEdgeInstances = true
				GUID_TIMESTATEMANAGER:
					new_parent.useTimeStateManager = true
				GUID_XTIMESTATE:
					new_parent.useTimeState = true
			state.add_fileID(new_parent, self)
			convert_properties(new_parent, self.keys)
			log_debug("AxNode3D: enabled feature for GUID " + guid + " on " + str(new_parent.name))
			return null  # No new child node; AxNode3D lives on the parent

		# Fall through to plugin system for non-AxNode3D MonoBehaviours
		var ret: Node = null
		for plugin in meta.get_enabled_plugins():
	...
	# Copies a String-typed AxNode3D property from Unity uprops SAFELY. Unity can
	# hand us an unresolved object-reference array for a property; str()-ing it
	# (the old code) baked artifacts like "[<null>, 0, <null>, <null>]" into
	# ThingInstanceLabel / ThingLabelOverride (XSG-59 straggler b). Only genuine
	# Unity STRING values are copied; anything else is dropped to "" with a warning
	# so a later RecalculateInstanceLabels regenerates a proper label instead.
	func _ustr_prop(uprops: Dictionary, key: String) -> String:
		var v: Variant = uprops.get(key, "")
		if typeof(v) == TYPE_STRING:
			return v
		log_warn("%s is not a String (got %s) — writing empty; check the Unity source object reference" % [key, str(v)], key)
		return ""
	...
	func convert_properties(node: Node, uprops: Dictionary) -> Dictionary:
		...
		# Fallback for prefab modifications where virtual objects lack m_Script
		if guid.is_empty() and (node is AxNode3D):
			guid = _infer_axnode_guid_from_uprops(uprops)

		# Only attempt AxNode3D property mapping if this is a known type and the node has the script
		if guid.is_empty() or not (node is AxNode3D):
			return outdict

		var ax: AxNode3D = node

		match guid:
			GUID_THINGLINK:
				if uprops.has("ThingLabelOverride"):
					ax.ThingLabelOverride = _ustr_prop(uprops, "ThingLabelOverride")
				if uprops.has("ThingNameOverride"):
					ax.ThingNameOverride = _ustr_prop(uprops, "ThingNameOverride")
				if uprops.has("PhysicalType"):
					var pt_idx: int = int(uprops.get("PhysicalType", 0))
					if pt_idx >= 0 and pt_idx < PHYSICAL_TYPES.size():
						ax.PhysicalType = PHYSICAL_TYPES[pt_idx]
					else:
						log_warn("PhysicalType index out of range: " + str(pt_idx))
				if uprops.has("ThingInstanceLabel"):
					ax.ThingInstanceLabel = _ustr_prop(uprops, "ThingInstanceLabel")
				if uprops.has("ObjectInstanceGuid"):
					ax.ThingGuid = str(uprops.get("ObjectInstanceGuid", ""))
				if uprops.has("PrefabPath"):
					ax.PrefabPath = str(uprops.get("PrefabPath", ""))
				if uprops.has("ParentOverride"):
					# Store the referenced GameObject name since cross-node refs are not trivially resolvable
					var parent_ref: Variant = uprops.get("ParentOverride")
					if typeof(parent_ref) == TYPE_ARRAY and parent_ref[1] != 0:
						var parent_obj = meta.lookup(parent_ref)
						if parent_obj != null:
							ax.ParentOverride_name = str(parent_obj.name)
						else:
							log_warn("ParentOverride: could not resolve reference " + str(parent_ref))

			GUID_LAYER:
				if uprops.has("Layers"):
					ax.Layers = str(uprops.get("Layers", ""))

			GUID_LOCKTRANSFORM:
				if uprops.has("LockX"):
					var lx_idx: int = int(uprops.get("LockX", 0))
					if lx_idx >= 0 and lx_idx < LOCKTRANSFORM_XZ.size():
						ax.LockX = LOCKTRANSFORM_XZ[lx_idx]
				if uprops.has("LockY"):
					var ly_idx: int = int(uprops.get("LockY", 0))
					if ly_idx >= 0 and ly_idx < LOCKTRANSFORM_Y.size():
						ax.LockY = LOCKTRANSFORM_Y[ly_idx]
				if uprops.has("LockZ"):
					var lz_idx: int = int(uprops.get("LockZ", 0))
					if lz_idx >= 0 and lz_idx < LOCKTRANSFORM_XZ.size():
						ax.LockZ = LOCKTRANSFORM_XZ[lz_idx]
				if uprops.has("Messages"):
					ax.Messages = str(uprops.get("Messages", ""))

			GUID_PIVOTOVERRIDE:
				if uprops.has("Rotation"):
					var rot: Variant = uprops.get("Rotation")
					if typeof(rot) == TYPE_VECTOR3:
						ax.PivotRotation = rot
					elif typeof(rot) == TYPE_DICTIONARY:
						ax.PivotRotation = Vector3(
							float(rot.get("x", 0)),
							float(rot.get("y", 0)),
							float(rot.get("z", 0)),
						)
				if uprops.has("CamDistanceMultiplier"):
					ax.CamDistanceMultiplier = float(uprops.get("CamDistanceMultiplier", 1.0))

			GUID_THINGEDGE:
				if uprops.has("Type"):
					var et_idx: int = int(uprops.get("Type", 0))
					if et_idx >= 0 and et_idx < THING_EDGE_TYPES.size():
						ax.EdgeFlags = ax.EdgeFlags | (1 << et_idx)
					else:
						log_warn("ThingEdgeType index out of range: " + str(et_idx))
				# Populate edge instance arrays (Vertex1 → Vertex2 relationship)
				var v1_ref: Variant = uprops.get("Vertex1")
				var v2_ref: Variant = uprops.get("Vertex2")
				if v1_ref != null and v2_ref != null and typeof(v1_ref) == TYPE_ARRAY and typeof(v2_ref) == TYPE_ARRAY:
					if v1_ref[1] != 0 and v2_ref[1] != 0:
						var v1_path: NodePath = meta.fileid_to_nodepath.get(v1_ref[1], meta.prefab_fileid_to_nodepath.get(v1_ref[1], NodePath()))
						var v2_path: NodePath = meta.fileid_to_nodepath.get(v2_ref[1], meta.prefab_fileid_to_nodepath.get(v2_ref[1], NodePath()))
						if v1_path != NodePath() and v2_path != NodePath():
							var edge_type_str: String = "None"
							if uprops.has("Type"):
								var eti: int = int(uprops.get("Type", 0))
								if eti >= 0 and eti < THING_EDGE_TYPES.size():
									edge_type_str = THING_EDGE_TYPES[eti].capitalize()
							var node_path: NodePath = meta.fileid_to_nodepath.get(self.fileID, meta.prefab_fileid_to_nodepath.get(self.fileID, NodePath()))
							var rel_v1: NodePath = NodePath("../" + str(v1_path)) if node_path == NodePath() else _relative_node_path(node_path, v1_path)
							var rel_v2: NodePath = NodePath("../" + str(v2_path)) if node_path == NodePath() else _relative_node_path(node_path, v2_path)
							ax.EdgeVertex1.append(rel_v1)
							ax.EdgeVertex2.append(rel_v2)
							ax.EdgeInstanceTypes.append(edge_type_str)
							var edge_val: int = int(uprops.get("Value", 0))
							ax.EdgeInstanceValues.append(edge_val)
						else:
							log_warn("ThingEdge: could not resolve Vertex1/Vertex2 node paths")

			GUID_THING3DLABEL:
				pass  # No serialized properties; useLabel3D toggle was set in create_godot_node

			GUID_ROUTINGWAYPOINT:
				pass  # No properties; isRoutingWaypoint toggle was set in create_godot_node

			GUID_TIMESTATEMANAGER:
				if uprops.has("SceneDateTime"):
					var sdt: Variant = uprops.get("SceneDateTime")
					if typeof(sdt) == TYPE_DICTIONARY and sdt.has("m_dateTime"):
						ax.SceneDateTime = str(sdt.get("m_dateTime", ""))
					elif typeof(sdt) == TYPE_STRING:
						ax.SceneDateTime = str(sdt)

			GUID_XTIMESTATE:
				if uprops.has("TimeStateChanges"):
					var so_ref: Variant = uprops.get("TimeStateChanges")
					if typeof(so_ref) == TYPE_ARRAY:
						var so_obj = meta.lookup(so_ref)
						if so_obj != null:
							var so_keys: Dictionary = so_obj.keys
							if so_keys.has("Notes"):
								ax.TimeStateNotes = str(so_keys.get("Notes", ""))
							if so_keys.has("StateChanges"):
								ax.TimeStateChangesJson = _serialize_time_state_changes(so_keys.get("StateChanges"))
						else:
							log_warn("XTimeState: could not resolve TimeStateChanges reference " + str(so_ref))

		return outdict

	func _serialize_time_state_changes(changes: Variant) -> String:
		...
		for change in changes:
			...
			var at_val: Variant = change.get("At", {})
			if typeof(at_val) == TYPE_DICTIONARY:
				entry["At"] = str(at_val.get("m_dateTime", ""))
			elif typeof(at_val) == TYPE_STRING:
				entry["At"] = str(at_val)
			var eo_idx: int = int(change.get("EnableObject", 0))
			if eo_idx >= 0 and eo_idx < TIMESTATE_ENABLE_TYPES.size():
				entry["EnableObject"] = TIMESTATE_ENABLE_TYPES[eo_idx]
			var items: Variant = change.get("TransformItems", [])
			if typeof(items) == TYPE_ARRAY:
				...
				for item in items:
					if typeof(item) != TYPE_DICTIONARY:
						continue
					var ti: Dictionary = {}
					var tt_idx: int = int(item.get("TransformType", 0))
					if tt_idx >= 0 and tt_idx < TIMESTATE_TRANSFORM_TYPES.size():
						ti["TransformType"] = TIMESTATE_TRANSFORM_TYPES[tt_idx]
					var val: Variant = item.get("Value")
					if typeof(val) == TYPE_VECTOR3:
						ti["Value"] = {"x": val.x, "y": val.y, "z": val.z}
					elif typeof(val) == TYPE_DICTIONARY:
						ti["Value"] = {"x": float(val.get("x", 0)), "y": float(val.get("y", 0)), "z": float(val.get("z", 0))}
					ti["Relative"] = bool(item.get("Relative", false))
					ti_arr.append(ti)
				if not ti_arr.is_empty():
					entry["TransformItems"] = ti_arr
			result.append(entry)
